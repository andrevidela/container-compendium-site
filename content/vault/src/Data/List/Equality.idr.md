...
Equality between two indexed types can be tricky, and that is the case with `List.All`. To address this challenge, we define a bespoke equality type between two `All` values where the index is the same.

````definition
Two `List.All` are equal when all of their elements are also equal.
```idris
public export
...
````proposition
We can convert from list equality to propositional equality.
```idris
export
listAllEqToEq :
