...
Using the product of categories we can define _Bifunctors_ a functor using
a product of categories in the codomain.
...
Given a functor $B : \cat{C_1} × \cat{C_2} → \cat{C_3}$ and $x \in \cat{C_1}$ we can apply $B$ to $x$ and obtain a functor $B(x) : \cat{C_2} → \cat{C_3}$.
```idris
public export
...
We can relate our notion of categorical products with the product of categories by stating that given a category $\cat{C}$ with products, we can define a functor $\cat{C} * \cat{C} → \cat{C}$ that maps each objects $|\cat{C}\times\cat{C}|$ to $|\cat{C}|$ using its product, and the product's universal property to combine morphisms.
```idris
public export
