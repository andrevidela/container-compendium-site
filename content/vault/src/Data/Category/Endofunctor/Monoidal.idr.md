...
there is a bifunctor $\text{EndoCat} × \text{EndoCat} → \text{EndoCat}$ where the map
...
```
````proposition
Given a category $\cat{C}$ and the category $\cat{E}$ of endofunctors on $\cat{C}$, there is
a bifunctor $(;_{\cat{E}}) : \cat{E} × \cat{E} → \cat{E}$ that composes endofunctors.
```idris
  monMultCompose :
...
````proposition
Given a category $\cat{C}$ and the category $\cat{E}$ of endofunctors over $\cat{C}$,
then $\cat{E}$ is monoidal wrt $;_{\cat{E}}$.
```idris
