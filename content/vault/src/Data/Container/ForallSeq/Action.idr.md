...
$\ContCart\times\Cont \to\Cont$ which we prove in this module. We refer to the
definition of Action from section$~\ref{def:action}$.
...
````proposition
$\forallSeq$ is an action of $\ContCart$ on $\Cont$
```idris
public export
