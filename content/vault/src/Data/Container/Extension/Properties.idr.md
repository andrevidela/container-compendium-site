...
The extension of containers is a crucial feature making them suitable for generic
data programming and has a few very nice propertie we explore here.

<!-- idris
public export
-->
````proposition
The extension of containers forms a bifuctor $⟦\_⟧\_ : \Cont × \Set → \Set$
...
Or equivalently, a functor $⟦\_⟧: \Cont → [Set,Set]$.
...
From this definition we recover functors in $\Set$ indexed by a container
...
````proposition
The extension functor $⟦\_⟧ :\Cont \to [\Set,\Set]$ is full and faithful.
```idris
...
- [?] Can we say that if we have a monad in the extension of the container then we have a monoid in Cont wrt to $\compose$?
