...
````proposition
$MaybeCont$ is a monoid in $\Cont$ with regards to $(*)$
```idris
...
This fact is actually the same as stating that $Maybe\ a \in \Set$ is a monoid
in $\Set$ with `Nothing` as the neutral and `(<|>)` as the multiplication.
...
-->
````proposition
$MaybeCont$ is a monoid in $\Cont$ with regards to $\rhd$
```idris
...
This fact is actually the same as stating that $Maybe \in \Set\to\Set$ is a monad in $\Set$.
...
````proposition
$MaybeCont$ is a monoid in $\ContCart$ with regards to $\rhd$
```idris
nothing : I =#> MaybeCont
