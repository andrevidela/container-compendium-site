...
We can partially apply the above bifunctor to obtain endofunctors in $\Cont$ given
any functor $M : \Set → \Set$

````proposition
Given a functor $F : \Set→\Set$ we obtain a functor $F • _ : \Cont → \Cont$
```idris
...
The inversion from $(•)$ means that every monad on $\Set$ becomes a _comonad_ in $\Cont$ when applied
with `(•)`.

````proposition
Given a monad $m : \Set → \Set$, the functor $m • _ : \Cont→\Cont$ is a comonad.
````
...
````proposition
Functorial application distributes to the right of $(\rhd)$ when `m` is a monad.
````
