...
````definition
The "Any" List functor in containers is given by $List (A, \bar{A}) = (xs : List\ A, Any\ \bar{A}\ xs)$
```idris
namespace Any
  public export
...
````definition
The "Any" List functor as a the $ListCont \rhd \_$ functor
```idris
public export
...
Proving that `Forany` is a functor is easy done by applying the $\rhd$ bifunctor to the $ListCont$ container.
...
````definition
The "All" List functor on containers is given by $List (A, \bar{A}) = (xs : List\ A, All\ \bar{A}\ xs)$
```idris
namespace All
  public export
  List : Container -> Container
...
This definition is isomorphic to the partially applied bifunctor $ListCont \forallSeq \_$.

````proposition
The "Forall" list functor on containers is given by $List (A, \bar{A}) = ListCont \forallSeq (A, \bar{A})$
```idris
public export
...
````lemma
$Forall\ a = ListCont ▶ a$ is Isomorphic to $List (A, \bar{A}) = (ls : List\ A, All\ \bar{A}\ ls)$
````

The statement that `Forall` is a functor is given by the partially applied action $ListCont \forallSeq \_$
