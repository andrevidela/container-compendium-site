...
```proposition
The $ListCont$ container is a monoid wrt to cartesian product $(*)$ in $\Cont$
```
...
This statement is equivalent to saying that $List\ a ∈ \Set$ is a monoid with
the empty list as neutral element and concatenation as multiplication.
...
```
```proposition
The $ListCont$ container is a monoid wrt composition $(\rhd)$ in $\ContCart$
```
```idris
...
This statement is equivalent to saying that $List ∈ \Set\to\Set$ is a monad in $\ContCart$.
