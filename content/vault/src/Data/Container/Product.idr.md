...
The cartesian product of container is distinct from the tensor product
in that the responses are combined with a coproduct, rather than a product.

````definition {label="def:product-cont"}
$(A,\bar{A})*(B,\bar{B}) = ((a,b): A*B , \bar{A}(a)+\bar{B}(b))$
```idris
...
and the coproduct on _types_ are bifunctors. That is why in the implementation
...
````proposition
There is a universal map $(c → a) × (c → b) → c → a *b$.
```idris
...
Additionally, the cartesian product provides a monoidal structure in $\Cont$

````proposition
$\Cont$ is monoidal in $(*)$ with $1$ as the neutral.
```idris
public export
