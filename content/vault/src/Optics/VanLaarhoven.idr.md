...
````proposition
The Store comonad $Store\ b\ a = b * b → a$ is isomorphic to
$Ex (b, b) \ a$
```idris
...
````proposition
The Pointed functor $Aff\ b\ a = a + b * (b → a$ is isomorphic to
$Ex Maybe(b, b) \ a$
```idris
namespace Affine
...
We introduce a new lens-like structure, the _monadic traversal_ where
the constraint is a _monad_.
